...
/**
 * Permite seleccionar y mover figuras de un documento de dibujo del editor.
 * <p>
 * 
 ...
 * seleccionarían todas las figuras confinadas en el interior de dicha área, o
 * podría bastar incluso con que el área rectangular así definida tocase a una
 * figura (la implementación concreta varía de unas aplicaciones a otras, y
 * dependería de cómo quisiéramos que funcionara nuestro editor).
 * <p>
 * 
 * Del mismo modo, cuando se hace clic sobre otra figura existente se limpia la
 * selección anterior que hubiera y pasaría a estar seleccionada la nueva figura
 * sobre la que se pulsó. Es decir, no cuenta, tampoco, por ahora, con la
 * posibilidad de seleccionar varias figuras mediante alguna combinación de la
 * pulsación de una tecla más el botón del ratón, como suele ser común en
 * cualquier editor gráfico (normalmente mediante la tecla [Control] en Windows
 * o [Command] en Mac más el botón del ratón).
 * <p>
 * 
...
public class SelectionTool implements Tool {
	...
	/**
	 * Las coordenadas con la posición actual del cursor. Inicialmente, el punto
	 * donde se pulsó con la herramienta de selección (es decir, las que se
	 * recibieron en la llamada a {@code clickOn(int x, int y)}; luego, las de
	 * los movimientos intermedios del cursor con la herramienta, a través del
	 * método {@code movedTo(int x, int y}.
	 */
	private int x, y;
	...
	public void clickOn(int x, int y) {
		...
		selectedFigure = editor.findFigureAt(x, y);
		if (selectedFigure != null) {
			editor.trace("Se seleccionó la figura: " + selectedFigure);
			editor.selectFigure(selectedFigure);
		} else {
	...
	/**
	 * Actualiza la posición actual del cursor a las nuevas coordenadas
	 * recibidas. Este método es llamado cuando se hace clic con el ratón así
	 * como cada vez que se mueve el cursor a una nueva posición. De esta forma,
	 * si hay una figura seleccionada, se sabe qué incremento hay que moverla
	 * (la diferencia entre la nueva posición del cursor recibida en el evento
	 * {@code movedTo(int x,
	 * int y)} y la posición anterior previamente guardada a través de este
	 * método.
	 * 
	 * @param x
	 *            la coordenada horizontal de la posición actual del cursor
	 * @param y
	 *            la coordenada vertical de la posición actual del cursor
	 */
